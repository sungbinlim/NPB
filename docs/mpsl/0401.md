...
- 가령 \\( \mathcal{X} \\) 가 MNIST 데이터셋, 즉 \\( 28 \times 28 \\) 픽셀 이미지 중 0 ~ 9 까지의 모든 숫자들의 이미지를 모은 집합이라면 그 원소는 \\( x \\) 는 개별 이미지를 말한다

![figure 1.1 20%]({{ site.baseurl }}/images/mpsl_Figure040101.png) \\( \in \\) ![figure 1.2]({{ site.baseurl }}/images/mpsl_Figure040102.png)
